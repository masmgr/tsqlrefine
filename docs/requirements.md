...
* SQL Server の T-SQL（**SQL Server 2012 以降**）
* 互換レベルは **設定ファイルで指定可能**（必要ならCLIで上書き）
* ソースコード上の SQL（ファイル、標準入力、VSCode等からの入力）

  * 対象は **純SQLのみ**（アプリコード内SQL文字列の抽出は対象外）
...
* `GO` / 複数バッチ: 対応する
* `CREATE PROC` 等の DDL: 対応する
* 動的SQL文字列（`EXEC(...)` 等）および **コメント/文字列リテラル内**は、文字列として扱い **解析対象外**とする
...
* **コメント、文字列、括弧内改行は変更しない**
* **インデントはスペース/タブの切り替えに対応**（既定は設定または入力踏襲）
...
* CLI の入出力仕様（JSON形式/終了コード）は `docs/cli.md` に集約する
...
* 静的解析ルール追加
  ...
  * **Rule のみ**プラグイン化可能にする（Formatter/Reporter はコア固定）
...
次にやると一気に設計が進むのは、**(A) ルールカテゴリ案と優先順位**、**(B) CLIの入出力仕様（JSON形式/終了コード）**、**(C) プラグインAPIの最小契約（Rule）** の3つ。

もし続けるなら、あなたの想定に寄せて「初期リリース（MVP）」を切るために、**最初に入れるルール10〜20個**と**フォーマットの最小セット**の叩き台を作成する。

関連ドキュメント（整備済）

* ルールカテゴリ案と優先順位: `docs/rules.md`
* CLI 入出力仕様（JSON/終了コード）: `docs/cli.md`
