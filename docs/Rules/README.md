...
> NOTE: This file is generated by `extract-all-rules.ps1`. Do not edit manually.

This document provides a comprehensive overview of all built-in rules in TsqlRefine. TsqlRefine includes 86 rules covering various aspects of T-SQL code quality, from correctness and safety to performance and style.
...
- **Warning Severity**: 49 rules (57%)
- **Information Severity**: 26 rules (30%)
...
The following 7 rules support automatic fixing:
...
2. [prefer-unicode-string-literals](style/prefer-unicode-string-literals.md) - Encourages Unicode string literals (N'...') to avoid encoding issues, using conservative safe-mode autofixes.
3. [require-as-for-column-alias](style/require-as-for-column-alias.md) - Column aliases should use the AS keyword
4. [require-as-for-table-alias](style/require-as-for-table-alias.md) - Table aliases should use the AS keyword
5. [require-explicit-join-type](style/require-explicit-join-type.md) - Disallows ambiguous JOIN shorthand; makes JOIN semantics explicit and consistent across a codebase.
6. [semantic/unicode-string](correctness/semantic-unicode-string.md) - Detects Unicode characters in string literals assigned to non-Unicode (VARCHAR/CHAR) variables, which may cause data loss.
7. [semicolon-termination](style/semicolon-termination.md) - SQL statements should be terminated with a semicolon
