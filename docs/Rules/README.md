...
This document provides a comprehensive overview of all built-in rules in TsqlRefine. TsqlRefine includes 82 rules covering various aspects of T-SQL code quality, from correctness and safety to performance and style.
...
- **Warning Severity**: 44 rules (54%)
- **Information Severity**: 24 rules (29%)
...
|----------|-------|-------------|
| **Correctness** | 23 | Detects code that may produce incorrect results or runtime errors |
...
The following 6 rules support automatic fixing:
...
2. [require-as-for-column-alias](style/require-as-for-column-alias.md) - Column aliases should use the AS keyword
3. [require-as-for-table-alias](style/require-as-for-table-alias.md) - Table aliases should use the AS keyword
4. [require-explicit-join-type](style/require-explicit-join-type.md) - Disallows ambiguous JOIN shorthand; makes JOIN semantics explicit and consistent across a codebase.
5. [semantic/unicode-string](correctness/semantic-unicode-string.md) - Detects Unicode characters in string literals assigned to non-Unicode (VARCHAR/CHAR) variables, which may cause data loss.
6. [semicolon-termination](style/semicolon-termination.md) - SQL statements should be terminated with a semicolon
