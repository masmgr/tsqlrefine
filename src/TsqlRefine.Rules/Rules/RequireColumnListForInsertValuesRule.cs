...
public sealed class RequireColumnListForInsertValuesRule : IRule
{
    ...
    private sealed class RequireColumnListForInsertValuesVisitor : TSqlFragmentVisitor
    {
            ...
            if (node.InsertSpecification?.InsertSource is ValuesInsertSource)
            {
                ...
                if (columns is null || columns.Count == 0)
                {
                    _diagnostics.Add(new Diagnostic(
                        ...
                        Data: new DiagnosticData("require-column-list-for-insert-values", "Correctness", false)
                    ));
                }
            }
