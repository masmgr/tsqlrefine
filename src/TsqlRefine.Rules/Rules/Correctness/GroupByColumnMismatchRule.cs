...
public sealed class GroupByColumnMismatchRule : DiagnosticVisitorRuleBase
{
    ...
    {
        private static readonly FrozenSet<string> s_aggregateFunctions = FrozenSet.ToFrozenSet(
        [
        ...
        {
            // Window functions with OVER clause are not aggregate in GROUP BY context,
            // but their internal column references don't need to be in GROUP BY
            ...
            return name != null && s_aggregateFunctions.Contains(name);
        }
