...
/// </summary>
public sealed class SetArithabortRule : IRule
{
    public RuleMetadata Metadata { get; } = new(
        RuleId: "set-arithabort",
        Description: "Files should start with SET ARITHABORT ON within the first 10 statements.",
    ...
    public IEnumerable<Diagnostic> Analyze(RuleContext context)
    {
        ...
        var foundArithabortOn = ScriptStatementAnalysisHelpers.AnyInFirstStatements(
            script,
            maxStatementsToCheck,
            static statement => statement is PredicateSetStatement
            {
                IsOn: true,
                Options: SetOptions.ArithAbort
            });

        if (!foundArithabortOn)
        {
            yield return ScriptStatementAnalysisHelpers.CreateFileStartDiagnostic(
                Metadata,
                "File should start with 'SET ARITHABORT ON' within the first 10 statements.");
        }
    }
