...
/// </summary>
public sealed class SetAnsiWarningsRule : IRule
{
    public RuleMetadata Metadata { get; } = new(
        RuleId: "set-ansi-warnings",
        Description: "Files should start with SET ANSI_WARNINGS ON within the first 10 statements.",
    ...
    public IEnumerable<Diagnostic> Analyze(RuleContext context)
    {
        ...
        var foundAnsiWarningsOn = ScriptStatementAnalysisHelpers.AnyInFirstStatements(
            script,
            maxStatementsToCheck,
            static statement => statement is PredicateSetStatement
            {
                IsOn: true,
                Options: SetOptions.AnsiWarnings
            });

        if (!foundAnsiWarningsOn)
        {
            yield return ScriptStatementAnalysisHelpers.CreateFileStartDiagnostic(
                Metadata,
                "File should start with 'SET ANSI_WARNINGS ON' within the first 10 statements.");
        }
    }
