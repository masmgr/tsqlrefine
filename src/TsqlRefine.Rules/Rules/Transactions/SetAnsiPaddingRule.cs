...
/// </summary>
public sealed class SetAnsiPaddingRule : IRule
{
    public RuleMetadata Metadata { get; } = new(
        RuleId: "set-ansi-padding",
        Description: "Files should start with SET ANSI_PADDING ON within the first 10 statements.",
    ...
    public IEnumerable<Diagnostic> Analyze(RuleContext context)
    {
        ...
        var foundAnsiPaddingOn = ScriptStatementAnalysisHelpers.AnyInFirstStatements(
            script,
            maxStatementsToCheck,
            static statement => statement is PredicateSetStatement
            {
                IsOn: true,
                Options: SetOptions.AnsiPadding
            });

        if (!foundAnsiPaddingOn)
        {
            yield return ScriptStatementAnalysisHelpers.CreateFileStartDiagnostic(
                Metadata,
                "File should start with 'SET ANSI_PADDING ON' within the first 10 statements.");
        }
    }
