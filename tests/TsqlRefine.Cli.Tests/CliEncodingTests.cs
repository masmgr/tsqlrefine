...
public sealed class CliEncodingTests
{
    ...
    [Fact]
    public async Task Format_Write_WithoutDetectEncoding_PreservesOriginalEncoding()
    {
        ...
        try
        {
            ...
            // Get original file size to verify encoding
            var originalBytes = await File.ReadAllBytesAsync(sqlPath);

            var stdout = new StringWriter();
            var stderr = new StringWriter();

            // Run format --write WITHOUT --detect-encoding
            var code = await CliApp.RunAsync(new[] { "format", "--write", sqlPath }, TextReader.Null, stdout, stderr);
    ...
    [Fact]
    public async Task Format_Write_WithoutDetectEncoding_PreservesUtf8WithBom()
    {
